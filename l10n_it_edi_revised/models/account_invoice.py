...
class AccountMove(models.Model):
    ...
    def invoice_generate_xml(self):
        for invoice in self:
            if invoice.l10n_it_einvoice_id and not (invoice.l10n_it_send_state in ['invalid', 'to_send']):
    ...
    def _export_as_xml(self):
        ...
        def format_unitprice(number):
            # Format the monetary values to avoid trailing decimals (e.g. 90.85000000000001).
            return float_repr(number, 5)
        ...
        null_tax_lines_id = self.invoice_line_ids.mapped(lambda x: x.tax_ids).filtered(lambda x: x.amount == 0)
        null_taxes = []

        for tax in null_tax_lines_id:
            null_taxes.append({
                'aliquota': "0.00",
                'natura':   tax.l10n_it_kind_exoneration,
                'reference': tax.l10n_it_law_reference,
                'importo': 0,
                'imponibile': sum(self.invoice_line_ids.filtered(lambda x: tax.id in x.tax_ids.ids).mapped('price_subtotal'))
            })
        ...
        template_values = {
            ...
            'format_unitprice': format_unitprice,
            ...
            'pdf_name': pdf_name,
            'dati_ddt': dati_ddt,
            'null_taxes': null_taxes
        }
        content = self.env.ref('l10n_it_edi_revised.account_invoice_it_FatturaPA_export').render(template_values)
        return content
    ...
    def _import_xml_invoice(self, tree):
        ...
        for body_tree in tree.xpath('//FatturaElettronicaBody'):
            ...
            with Form(self.with_context(default_type=type)) as invoice_form:
                    ...
                    for element in elements:
                        with invoice_form.invoice_line_ids.new() as invoice_line_form:
                            ...
                            if not invoice_line_form.account_id:
                                invoice_line_form.account_id = self.env['account.account'].browse(2213)
                ...
                for discount in discount_list:
                    with invoice_form.invoice_line_ids.new() as invoice_line_form_discount:
                        ...
                        if not invoice_line_form_discount.account_id:
                            invoice_line_form_discount.account_id = self.env['account.account'].browse(2213)
            ...
            for line in new_invoice.invoice_line_ids:
                if (line.price_unit == 0) and (not line.product_id):
                    line.write({
    ...
    def _helper_load_invoice_from_attachment(self, attachment_id):
        document = self.env['documents.document'].browse(attachment_id)

        #data = document.datas.encode('utf-8')
        message = base64.b64decode(document.datas)
        #message = message_bytes.decode('utf-8')
        
        invoice = self.env['fetchmail.server']._create_invoice_from_mail(message, document.name, document.partner_id.l10n_it_pec_email or 'sdi01@pec.fatturapa.it')
        document.res_model = 'account.move'
        document.res_id = invoice.ids[0]
        document.partner_id = invoice[0].partner_id
...
class AccountTax(models.Model):
    ...
    l10n_it_kind_exoneration = fields.Selection(selection=[
            ...
            ("N2", "[N2] Non soggette - DEPRECATO"),
            ("N2.1", "[N2.1] Non soggette - Ai sensi degli articoli da 7 a 7-septies del D.P.R. n. 633/1927"),
            ("N2.2", "[N2.2] Non soggette - Altri Casi"),
            ("N3", "[N3] Non imponibili - DEPRECATO"),
            ...
            ("N3.5", "[N3.5] Non imponibili - A Seguito di Dichiarazioni di Intento"),
            ...
            ("N6", "[N6] Inversione contabile - DEPRECATO"),
            ...
            ("N6.5", "[N6.4] Inversione contabile - Operazioni Settore Energetico"),
            ("N6.6", "[N6.4] Inversione contabile - Altri Casi"),
