...
class ir_attachment(models.Model):
    ...
    def _find_or_create_root_directory(self):
        ...
        try:
            ...
            Config = self.env['ir.config_parameter'].sudo()
            dir_key = Config.get_param('onedrive_root_dir_id', '')
            dir_path = Config.get_param('onedrive_root_dir_path', 'Odoo Attatchments')


            if dir_key:
                try:
                    #in try, since the folder might be removed in meanwhile
                    client.get_drive_item_o(drive_id=drive, drive_item_id=dir_key).get("id")
                except:
                    dir_key = False
                    self._context.get("s_logger").warning(
                        u"The attachments root directory '{}' has been removed from Onedrive. Creating a new one".format(dir_path)
                    )
            if not dir_key:
                res = False, False
                """
                dir_key = mkdir(client, drive, False, dir_path)
                Config.set_param('onedrive_root_dir_id', dir_key)
                Config.set_param('onedrive_delta_url', '')
                self._context.get("s_logger").debug(u"The attachments root directory '{}' is created in OneDrive".format(dir_path))
                """
            res = dir_key, dir_path
        except Exception as error:
            res = False, False
            self._context.get("s_logger").error(
                u"The attachments root directory '{}' can't be created in OneDrive. Reason: {}".format(dir_path, error)
            )
        return res
